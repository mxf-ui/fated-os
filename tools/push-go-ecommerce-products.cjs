...
const files = [
  'js/games/go/00-state-init.js',
  'js/games/go/02-live-danmaku.js',
  'js/games/go/03-ecommerce.js',
  'css/modules/09-games.css',
  'css/modules/05-wechat.css',
  'js/main/chat/01-thread-render.js',
  'js/main.js',
  'js/main/05-chat-ai-persona.js',
  'tools/verify-go-ecommerce-products.mjs'
];
...
(async () => {
  ...
  const commit = await gh('POST', '/repos/' + owner + '/' + repo + '/git/commits', {
    message: 'Improve GO livestream product cards',
    tree: tree.sha,
    parents: [baseSha]
  });
