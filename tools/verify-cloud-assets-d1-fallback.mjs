...
assert(/function getAssetBucket\(env\)/.test(assetApi) && /env && env\.ASSET_BUCKET/.test(assetApi) && /typeof bucket\.put === 'function'/.test(assetApi), 'asset API should only use a real R2 ASSET_BUCKET binding');
